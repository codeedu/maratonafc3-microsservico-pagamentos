...
func SubscriptionHandler(request events.APIGatewayProxyRequest) (Response, error) {
	...
	transaction := domain.NewTransactionRequest()
	...
	transactionResponse, err := application.ProcessTransaction(transaction)

	if err != nil {
		return returnResponse(err.Error()), nil
	}

	transactionJson, err := json.Marshal(transactionResponse)
...
func process(transaction *domain.TransactionRequest) (Response, error) {
	...
	if transaction.Gateway.Name == "pagar.me" {
		...
		pagarme.SubscriptionEndPoint = "https://api.pagar.me/1/transactions"
		err := pagarme.Process()
